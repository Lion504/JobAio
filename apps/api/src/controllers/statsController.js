...
export const getJobStatsController = async (req, res, next) => {
  try {
    ...
    const topSkills = await OriginalJob.aggregate([
      ...
      { $limit: 8 },
      {
        $project: {
          ...
          percentage: {
            $round: [
              { $multiply: [{ $divide: ["$count", totalJobs] }, 100] },
              1,
            ],
          },
        },
    ...
    const languages = await OriginalJob.aggregate([
      {
        $project: {
          hasEnglish: {
            $in: ["English", { $ifNull: ["$language.required", []] }],
          },
          hasFinnish: {
            $in: ["Finnish", { $ifNull: ["$language.required", []] }],
          },
          hasSwedish: {
            $in: ["Swedish", { $ifNull: ["$language.required", []] }],
          },
        },
      },
      {
          ...
          stats: [
            {
              ...
              percentage: {
                $round: [
                  { $multiply: [{ $divide: ["$English", totalJobs] }, 100] },
                  1,
                ],
              },
            },
            {
              ...
              percentage: {
                $round: [
                  { $multiply: [{ $divide: ["$Finnish", totalJobs] }, 100] },
                  1,
                ],
              },
            },
            {
              ...
              percentage: {
                $round: [
                  { $multiply: [{ $divide: ["$Swedish", totalJobs] }, 100] },
                  1,
                ],
              },
            },
    ...
    const educationDistribution = await OriginalJob.aggregate([
      ...
      }, // Some jobs might not have education
