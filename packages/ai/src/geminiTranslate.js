...
export async function coreTranslateStructured(
  ...
  try {
    ...
    if (!replyText) {
      console.warn(
        `[coreTranslateStructured] Empty reply received for ${targetLang}.`,
      );
    ...
    if (
      ...
      return coreTranslateStructured(
        jobData,
        targetLang,
        sourceLang,
        retries - 1,
    ...
    console.error(
      `[coreTranslateStructured] Final translation failure for ${targetLang}:`,
      err.message || err,
    );
...
export async function translateJob(job) {
  ...
  if (sourceLang !== ENGLISH_LANG) {
    ...
    try {
      pivotJobData = await coreTranslateStructured(
        sourceJobData,
        ENGLISH_LANG,
        sourceLang,
      );
      ...
      console.warn(
        `[Pivot] Pivot failed, falling back to original source text for subsequent translations.`,
      );
    }
  ...
  for (const lang of TARGET_LANGUAGES) {
    ...
    try {
      const translatedData = await coreTranslateStructured(
        forwardSourceData,
        lang,
        effectiveSourceLang,
      );
